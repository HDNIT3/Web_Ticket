...
import { getMovies, createMovie, updateMovie, deleteMovie, getGenres } from '../../services/movie.api'
import { buildMovieFromForm } from '../../util/movieBuilder'
import { createGenreOptions, getGenreLabels } from '../../util/genreFactory'
import { movieEventBus } from '../../util/movieEventBus'
...
function AdminMovies() {
  ...
  const fetchMovies = useCallback(async (targetPage = 0, kw = '', genreIds = [], status = '') => {
    setLoading(true)
    setError('')
    try {
      ...
      const apiPage = res?.currentPage ?? 1
      setPage(apiPage > 0 ? apiPage - 1 : 0)
  ...
  const openEdit = (movie) => {
    ...
    if (movie.releaseDate) {
      const d = parseDateOnly(movie.releaseDate)
      if (d) {
    ...
    setForm({
      ...
      genreIds: (movie.genres ?? []).map((g) => g?._id ?? g?.id),
    })
    setFormError('')
    setShowModal(true)
  }
  ...
  const checkUrlFields = () => {
    if (form.posterUrl && !isValidHttpUrl(form.posterUrl)) {
      setFormError('URL poster không hợp lệ. Vui lòng dùng link bắt đầu bằng http/https.')
      return false
    }
    if (form.trailerUrl && !isValidHttpUrl(form.trailerUrl)) {
  ...
  const handleSave = async (e) => {
    ...
    if (!form.genreIds || form.genreIds.length === 0) {
      setFormError('Vui lòng chọn ít nhất một thể loại phim.')
      return
    }
    ...
    try {
      ...
      if (editingId) {
        ...
        movieEventBus.emitUpdated(res?.data ?? res)
      } else {
        const res = await createMovie(payload)
        notifySuccess(res?.message ?? 'Thêm phim thành công.')
        movieEventBus.emitCreated(res?.data ?? res)
      }
      closeModal()
      fetchMovies(page, keyword, selectedGenreIds, selectedStatus)
    } catch (err) {
    ...
    selectedGenreIds.length > 1
      ? movies.filter((movie) => (
          selectedGenreIds.every((id) =>
            (movie.genres ?? []).some((genre) => String(genre?._id ?? genre?.id) === String(id))
          )
        ))
      : movies
  ...
  const handleSearch = (e) => {
    ...
    fetchMovies(0, nextKeyword, selectedGenreIds, selectedStatus)
  }
  ...
  const handleGenreFilterToggle = (genreId) => {
    ...
    fetchMovies(0, keyword, nextGenreIds, selectedStatus)
  }
  
  const handleStatusChange = (e) => {
    const val = e.target.value
    setSelectedStatus(val)
    fetchMovies(0, keyword, selectedGenreIds, val)
  }
        ...
        <div className="card-body p-3 p-md-4 d-flex flex-column">
          <div className="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-3 mb-4">
            <div>
              ...
              <p className="text-secondary mb-0">Danh sách phim trong hệ thống.</p>
            </div>
            <div className="d-flex gap-2">
              <a href={window.location.hash.startsWith('#/staff') ? '#/staff/genres' : '#/admin/genres'} className="btn btn-outline-secondary">
                📁 Thể loại
              </a>
              <a href={window.location.hash.startsWith('#/staff') ? '#/staff/showtimes' : '#/admin/showtimes'} className="btn btn-outline-secondary">
                🎬 Lịch chiếu
              </a>
          ...
          {!loading && sortedMovies.length > 0 && (
            <div className="row g-4">
              {sortedMovies.map((movie, idx) => {
                const id = movie?._id ?? movie?.id
                const isAudioPreview = audioPreviewMovieId === id
                    ...
                    <div
                      ...
                      onPointerEnter={() => {
                        if (trailerPreviewUrl) {
                          setPreviewMovieId(id)
                        }
                      }}
                      onPointerLeave={() => {
                        setPreviewMovieId((prev) => (prev === id ? null : prev))
                        setAudioPreviewMovieId((prev) => (prev === id ? null : prev))
                      }}
                    >
                      <div
                      ...
                      >
                        ...
                        {previewMovieId === id && movie.trailerUrl && !isAudioPreview && (
                      ...
                      <div className="card-body d-flex flex-column">
                        ...
                        <div className="mt-auto d-grid gap-2">
                          ...
                          >
                            {deletingId === id ? 'Đang xoá...' : 'Xoá phim'}
                          </button>
                        </div>
                      </div>
                    </div>
            ...
            <div className="d-flex justify-content-between align-items-center mt-auto pt-3">
              ...
              <div className="d-flex gap-2">
                <button
                  ...
                  onClick={() => fetchMovies(page - 1, keyword, selectedGenreIds, selectedStatus)}
                >
                  Trước
                </button>
                <button
            ...
            <div className="modal-content">
              ...
              <form onSubmit={handleSave}>
                <div className="modal-body" style={{ maxHeight: '68vh', overflowY: 'auto' }}>
                  <div className="row g-3">
                    ...
                    <div className="col-12">
                      ...
                      <div className="d-flex flex-wrap gap-3 p-3 border rounded bg-light">
                        {genreOptions.length === 0 && (
                          <span className="text-secondary small">Không có thể loại nào. Hãy tạo thể loại trước.</span>
                        )}
                        {genreOptions.map((opt) => (
                          <div key={opt.value} className="form-check mb-0">
                            <input
                              type="checkbox"
                              className="form-check-input"
                              id={`movie-genre-${opt.value}`}
                              checked={form.genreIds.includes(opt.value)}
                              onChange={() => handleGenreToggle(opt.value)}
                            />
                            <label className="form-check-label" htmlFor={`movie-genre-${opt.value}`}>
                              {opt.label}
                            </label>
                          </div>
                        ))}
                  ...
                  {trailerEmbedUrl && (
                    <div className="mt-3">
                      <label className="form-label small text-secondary">Bản xem trước Trailer</label>
                      <iframe
                        src={trailerEmbedUrl}
