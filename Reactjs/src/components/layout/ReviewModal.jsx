...
export default function ReviewModal({ isOpen, bookingId, existingReview, onClose, onSubmitSuccess, showToast }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (existingReview) {
        ...
        showToast('Đánh giá phim thành công!', 'success')
      }
      onSubmitSuccess()
    } catch (err) {
      showToast(err.message || 'Lỗi xử lý đánh giá.', 'error')
    } finally {
      setSubmitting(false)
    }
  }
      ...
      >
        <div className="d-flex justify-content-between align-items-center border-bottom border-secondary border-opacity-30 pb-3 mb-3">
          <h5 className="fw-bold text-light mb-0">
            {existingReview ? '✏️ CHỈNH SỬA ĐÁNH GIÁ' : '⭐ ĐÁNH GIÁ PHIM'}
          </h5>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="d-flex justify-content-end gap-2 border-top border-secondary border-opacity-30 pt-3">
            <button type="button" className="btn btn-secondary px-3 rounded-3" onClick={onClose}>
              Hủy bỏ
            </button>
